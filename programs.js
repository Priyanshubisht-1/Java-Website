document.addEventListener("DOMContentLoaded", () => {
  ...
  const navToggle = () => {
    if (navLinks && burger) {
      ...
      body.style.overflow = navLinks.classList.contains("nav-active")
        ? "hidden"
        : "auto";
    }
  };
  ...
  async function loadPrograms(category) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Could not find ${category}.json. File not found (404).`
        );
      }
      ...
      if (programs.length === 0) {
        programContent.innerHTML =
          '<p class="no-match-msg">No programs found in this category yet. Check back soon!</p>';
        return;
      }
